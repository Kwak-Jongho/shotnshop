...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (NAVER_ID && NAVER_SECRET) {
      const productName = parsed.recognized || parsed.product_list?.[0]?.name || query;
      const naverSites = await searchNaver(NAVER_ID, NAVER_SECRET, productName);
...
async function searchNaver(clientId, clientSecret, query) {
  try {
    const url = `https://openapi.naver.com/v1/search/shop.json?query=${encodeURIComponent(query)}&display=50&sort=asc`;
